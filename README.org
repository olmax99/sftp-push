...
[[file:images/helm-plus-metabase.png][file:./images/sftppush_concurrency_design.png]]
...
#+BEGIN_EXAMPLE
All source directories are determined by:
      <defaults.userpath> + <watch.source.name> + <watch.source.paths>
#+END_EXAMPLE

=$HOME/.sftppush.yaml|sftppush/config.yaml= 
#+BEGIN_SRC yaml 
defaults:
  userpath:
watch:
...
# LAUNCH WITH CUSTOM CONFIG FILE PATH
$ vim config.yaml
$ ./bin/sftppush-0.1.0-linux_amd64 --config config.yaml

# OVERWRITE USER DIRECTORIES - by defaults set to /home
...
$ ./bin/sftppush-0.2.0-linux_amd64 -c config.yaml

# EXAMPLE: Run without config with two sources
$ ./bin/sftppush-0.2.0-linux_amd64 watch \
  --source='name=sftpuser1,"paths=device1/data,device2/data",s3target=olmax-test-sftppush-126912' \
  --source='name=sftpuser2,"paths=device1/data,device2/data",s3target=olmax-test-sftppush-126912
#+END_SRC 
